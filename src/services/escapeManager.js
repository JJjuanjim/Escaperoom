...
class EscapeManager {
  ...
  createRoom(hostPlayer, chosenScenarioId = null) {
    const code = this.generateRoomCode();
    let selectedScenario = this.scenarios.find(s => s.id === chosenScenarioId);
    if (!selectedScenario) {
      // Elegir escenario aleatorio si no se especifica
    ...
    const room = {
      ...
      currentLevelIndex: 0, // 0 = Nivel 1, 1 = Nivel 2, 2 = Nivel 3
      ...
      cableSequenceState: [],
      sharedCableSequence: [],
      valvesState: new Map(), // playerId -> number
      chatMessages: [],
      completedPuzzles: [],
      completedLevels: [],
      unlockedClues: []
  ...
  joinRoom(code, player) {
    ...
    if (room.status === 'PLAYING') {
      const existingPlayer = room.players.find(p => p.name.trim().toLowerCase() === player.name.trim().toLowerCase());
  ...
  // Aprobar reincorporación de un jugador a la misión activa con penalización de 10s
  approveRejoin(code, newSocketId, playerName, avatar) {
    const room = this.getRoom(code);
    if (!room || room.status !== 'PLAYING') return null;

    const existingIndex = room.players.findIndex(p => p.name.trim().toLowerCase() === playerName.trim().toLowerCase());
    if (existingIndex !== -1) {
    ...
    // Penalización de -10 segundos por reincorporación
  ...
  changeScenario(code, scenarioId) {
    const room = this.getRoom(code);
  ...
  submitPuzzleAnswer(code, playerId, puzzleId, answer) {
    const room = this.getRoom(code);
    ...
    if (isCorrect) {
      ...
      // Avanzar al siguiente puzzle o nivel
      if (room.currentPuzzleIndex + 1 < currentLevel.puzzles.length) {
        ...
        if (room.currentLevelIndex + 1 < room.levelsData.length) {
          ...
          // ¡Todos los niveles completados! Escape exitoso
      ...
      // Penalizar con 15 segundos por intento fallido
  ...
  // Sincronización en tiempo real de fusibles de cables compartidos
  pushCableColor(code, color) {
    const room = this.getRoom(code);
    if (!room || room.status !== 'PLAYING') return null;
  ...
  updateValve(code, playerId, pressureValue) {
    const room = this.getRoom(code);
    if (!room || room.status !== 'PLAYING') return null;
    ...
    let totalPressure = 0;
    room.valvesState.forEach(val => totalPressure += val);
  ...
  restartMission(code) {
    ...
    // Escoger escenario aleatorio o mantener el actual
  ...
  getSanitizedRoomState(room, playerId) {
    ...
    const valvesArray = [];
    room.valvesState.forEach((val, pId) => {
...
module.exports = new EscapeManager();
